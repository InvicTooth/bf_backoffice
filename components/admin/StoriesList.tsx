...
export default function StoriesList({ stories }: { stories: { id: number, title: string }[] | null }) {
  return (
    <div class="flex flex-col w-56 border-r border-gray-300">
      ...
      <div class="flex flex-col flex-grow p-4 overflow-auto">
        {stories?.map((story) => (
          <a class="flex items-center flex-shrink-0 h-10 px-2 text-sm font-medium rounded hover:bg-gray-300" href={`/admin/stories/${story.id}`}>
            <span class="leading-none">{story.title}</span>
          </a>
        ))}
        
        <a class="flex items-center flex-shrink-0 h-10 px-3 mt-auto text-sm font-medium bg-gray-200 rounded hover:bg-gray-300" href="/admin/stories/new">
          <img class='w-5 h-5' src='/images/admin/plus.svg' alt='Add Item' />
          <span class="ml-2 leading-none">새 스토리</span>
        </a>
      </div>
    </div>
  );
